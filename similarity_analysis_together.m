...
if match_data
    saveas(gcf,[output_dir datasets{d} '_similarityMat_matchedData_' num2str(amt_data) '.tiff'],'tiff');
else
    saveas(gcf,[output_dir datasets{d} '_similarityMat_unMatchedData.tiff'],'tiff');
end
close('all');
...
elseif strcmpi(dataset, 'Lifespan-FSU')
    ...
    sessions = 5;
elseif strcmpi(dataset, 'iNet-RO1')
    subject = {'INET001', 'INET002', 'INET003', 'INET005', 'INET006','INET010','INET026','INET042'};
    sessions = 4;
